...
def load_wm811k(pkl_path):
    ...
    if not os.path.exists(pkl_path):
        print(f"WARNING: WM-811K file not found at '{pkl_path}'.")
        print("Generating synthetic base wafer maps for pipeline execution...")
        
        def create_synth_wafer(grid_size=26, has_defect=True):
            r = grid_size / 2.0
            y, x = np.ogrid[:grid_size, :grid_size]
            dist_from_center = np.sqrt((x - r)**2 + (y - r)**2)
            circle_mask = dist_from_center <= (r - 1)
            wm = np.zeros((grid_size, grid_size), dtype=int)
            wm[circle_mask] = 1 # pass
            if has_defect:
                # Add synthetic defect pattern (e.g. edge ring or center spot)
                defect_mask = circle_mask & (dist_from_center > (r - 4)) & (np.random.random((grid_size, grid_size)) < 0.35)
                wm[defect_mask] = 2 # fail
            return wm

        labeled_records = [{"waferMap": create_synth_wafer(26, has_defect=True), "failureType": np.array([["Center"]])} for _ in range(200)]
        none_records = [{"waferMap": create_synth_wafer(26, has_defect=False), "failureType": np.array([["none"]])} for _ in range(200)]
        return pd.DataFrame(labeled_records), pd.DataFrame(none_records)
